...
def main() -> None:
    ...
    st.markdown('<div class="app-title">HR Cost Forecast (10-Year)</div>', unsafe_allow_html=True)
    ...
    with col_a:
        ...
        provenance = {
            "Provider": meta_raw.get("provider_id", "unknown"),
            "Metric": meta_raw.get("metric_name", "unknown"),
            ...
            "Filters": json.dumps(meta_raw.get("filters_used", {})),
            "Last refresh": meta.last_refresh_time,
        }
        st.markdown(
            f"""
            <div class="meta-grid">
              <div class="meta-key">Provider</div><div class="meta-val">{provenance["Provider"]}</div>
              <div class="meta-key">Metric</div><div class="meta-val">{provenance["Metric"]}</div>
              <div class="meta-key">Unit</div><div class="meta-val">{provenance["Unit"]}</div>
              <div class="meta-key">Currency</div><div class="meta-val">{provenance["Currency"]}</div>
              <div class="meta-key">Measure</div><div class="meta-val">{provenance["Measure"]}</div>
              <div class="meta-key">Aggregation</div><div class="meta-val">{provenance["Aggregation"]}</div>
              <div class="meta-key">Filters</div><div class="meta-val">{provenance["Filters"]}</div>
              <div class="meta-key">Last refresh</div><div class="meta-val">{provenance["Last refresh"]}</div>
            </div>
            """,
            unsafe_allow_html=True,
        )
        ...
        if st.button("Refresh from SAC", use_container_width=True):
            ...
            if result["ok"]:
                if result["refresh_used_cache"]:
                    st.warning("Refresh failed; using cached data.")
                else:
    ...
    scenario_wide = scenarios.pivot(index="date", columns="scenario", values="yhat")
    ...
    for idx, name in enumerate(preset_names):
        ...
        with preset_cols[idx]:
            ...
            st.caption(
                ...
                f"shock {preset['params'].shock_pct:+.2f} | "
                f"start {preset['params'].shock_start_year or '-'} | "
                f"dur {preset['params'].shock_duration_months or 0}m | "
                f"drift {preset['params'].drift_pp_per_year:+.2f}"
            )
    ...
    if selected not in PRESETS_V2:
        selected = "base"
        st.session_state["selected_preset"] = "base"
    preset_params = PRESETS_V2[selected]["params"]
    ...
    if "pending_override" in st.session_state:
        ...
        st.session_state["growth_delta_pp_widget"] = float(pending["growth_delta_pp"])
        st.session_state["shock_pct_widget"] = float(pending["shock_pct"])
        st.session_state["drift_pp_widget"] = float(pending["drift_pp"])
    ...
    if "override_defaults" not in st.session_state:
        st.session_state["override_defaults"] = {
            "growth_delta_pp": float(preset_params.growth_delta_pp_per_year),
            "shock_pct": float(preset_params.shock_pct),
            "drift_pp": float(preset_params.drift_pp_per_year),
    ...
    with st.form("override_form"):
        growth_delta_pp = st.slider(
            "Growth delta (pp per year)",
            -0.5,
            0.5,
            float(defaults["growth_delta_pp"]),
            0.01,
            key="growth_delta_pp_widget",
        )
    ...
    if suggestion:
        ...
        st.markdown(
            ...
            - growth delta: {s["growth_delta_pp_per_year"]:+.2f} pp/yr
            - shock start year: {s["shock_start_year"] or "n/a"}
            - shock pct: {s["shock_pct"]:+.2f}
            - shock duration: {s["shock_duration_months"]} months
            - drift: {s["drift_pp_per_year"]:+.2f} pp/yr
            """
        )
        with st.container():
        ...
        def _apply_suggestion() -> None:
            ...
            st.session_state["pending_override"] = {
                "growth_delta_pp": float(s["growth_delta_pp_per_year"]),
                "shock_pct": float(s["shock_pct"]),
                "drift_pp": float(s["drift_pp_per_year"]),
